...
tags: ["query:mas-gen"]
score: 6.0
evidence: 用于智能体搜索和任务分解的POMDP框架
tldr: 本研究针对大语言模型（LLM）在处理超长上下文环境（如大规模代码库）时面临的搜索效率低、记忆退化等问题，提出了“上下文收集决策过程”（CGDP）框架。该框架将代理搜索建模为部分可观测马尔可夫决策过程（POMDP），并引入了基于谓词的显式信念状态和程序化穷尽门控。实验表明，该方法在提升多跳推理能力的同时，显著降低了计算成本，为构建高效、模块化的代理搜索系统提供了理论指导。
selection_source: fresh_fetch
motivation: LLM代理在处理超出上下文窗口的海量信息时，常因缺乏显式搜索状态管理而导致重复劳动或过早停止。
method: 将代理搜索形式化为CGDP框架，并设计了基于谓词的持久化信念状态和用于检测搜索停滞的程序化穷尽门控。
result: "在多个问答领域中，该方法将多跳推理性能提升了11.4%，并节省了高达39%的Token消耗。"
conclusion: 将LLM代理循环视为CGDP过程，有助于设计出更模块化、更高效且互不干扰的代理搜索增强方案。
---

## 摘要
大语言模型 (LLM) 智能体被部署在复杂的环境中——例如海量代码库、企业数据库和对话历史——在这些环境中，相关状态远超其上下文窗口。为了在这些空间中导航，智能体必须迭代地探索环境以寻找相关信息。然而，在缺乏显式基础设施的情况下，智能体的工作记忆可能会退化为搜索状态的有损表示，导致冗余工作（如重复循环）和过早停止。在这项工作中，我们将这一挑战形式化为上下文收集决策过程 (CGDP)，这是一种专门的部分可观测马尔可夫决策过程 (POMDP)，其中智能体的目标是自适应地细化其信念状态，以隔离任务所需的信息。我们将 LLM 的行为建模为该 CGDP 中的近似汤普森采样 (Thompson Sampling)，并引入了一种基于谓词的方法，将 LLM 的隐式搜索分解为显式且模块化的操作。随后，我们为迭代式 LLM 智能体推导了两种即插即用的干预措施：一种持久的、基于谓词的信念状态，它在限制上下文的同时保留了多跳推理能力；以及一个程序化的穷尽门控，用于在不提前停止的情况下终止无成效的搜索。在四种方法和三个问答领域中，我们通过实验验证了用基于 CGDP 的信念状态替换 LLM 的隐式状态，可将多跳推理能力提高多达 11.4%；而模块化的程序化穷尽检测在不降低智能体性能的情况下，节省了多达 39% 的 token。最终，我们认为将 LLM 智能体循环框架化为 CGDP 可以指导代理式搜索工具链中模块化、无干扰改进的设计。
