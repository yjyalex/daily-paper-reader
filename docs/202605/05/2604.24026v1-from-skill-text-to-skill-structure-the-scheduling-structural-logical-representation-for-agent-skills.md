...
tags: ["query:mas-gen"]
score: 6.0
evidence: 智能体任务分解中技能知识的显式表示
tldr: 针对当前大模型智能体技能表示过于依赖非结构化文本、难以被机器高效管理和推理的问题，本文提出了一种名为SSL（调度-结构-逻辑）的结构化表示方法。该方法借鉴经典语言知识表示理论，将技能解构为调度信号、执行结构和逻辑动作三个层次。实验表明，SSL在技能发现和风险评估任务中显著优于纯文本基准，提升了智能体技能的可搜索性、可审查性和可重用性。
selection_source: fresh_fetch
motivation: 现有的智能体技能多以非结构化文本形式存在，导致机器在处理调用接口、执行逻辑和副作用时难以进行有效推理。
method: 提出SSL表示法，通过LLM归一化器将技能文本解构为调度层、场景结构层和逻辑动作层。
result: 在技能发现任务中MRR从0.573提升至0.707，在风险评估任务中宏F1值从0.744提升至0.787。
conclusion: 显式的、基于源数据的结构化表示能显著增强智能体技能的可操作性，是迈向更可检查和可重用技能管理的重要一步。
---

## 摘要
大语言模型（LLM）智能体日益依赖可重用的技能，即结合了指令、控制流、约束和工具调用的能力包。然而，在大多数当前的智能体系统中，技能仍由重文本的产物（artifacts）表示，包括 SKILL.md 风格的文档和结构化记录，其机器可用的证据在很大程度上仍嵌入在自然语言描述中。这给以技能为中心的智能体系统带来了挑战：管理技能集合以及利用技能支持智能体，都需要对调用接口、执行结构和具体的副作用进行推理，而这些内容往往纠缠在单一的文本表层中。因此，技能知识的显式表示可能有助于使这些产物更易于机器获取和利用。借鉴 Schank 和 Abelson 关于语言知识表示的经典著作中的记忆组织包（Memory Organization Packets）、脚本理论（Script Theory）和概念依赖（Conceptual Dependency），我们引入了据我们所知首个针对智能体技能产物的结构化表示，它解耦了技能级调度信号、场景级执行结构以及逻辑级动作和资源使用证据：即调度-结构-逻辑（Scheduling-Structural-Logical, SSL）表示。我们使用基于 LLM 的归一化器实例化了 SSL，并在技能发现（Skill Discovery）和风险评估（Risk Assessment）两个任务的技能语料库上对其进行了评估，其表现显著优于纯文本基线：在技能发现中，SSL 将 MRR 从 0.573 提高到 0.707；在风险评估中，它将宏 F1 从 0.744 提高到 0.787。这些发现表明，显式的、基于源的结构使智能体技能更易于搜索和审查。它们还表明，SSL 最好被理解为迈向更具可检查性、可重用性和操作可行性的智能体系统技能表示的务实一步，而非一个成熟的标准或管理与使用技能的端到端机制。
