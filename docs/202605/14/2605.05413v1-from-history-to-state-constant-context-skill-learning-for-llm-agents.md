...
## 摘要
大语言模型（LLM）智能体越来越多地被用于操作浏览器、文件、代码和工具，使得个人助手成为一个自然的部署目标。然而，个人助手面临着隐私-成本-能力的权衡：云端模型能很好地执行多步工作流，但会将敏感的中间上下文暴露给外部 API；而本地模型虽然保护了隐私，但可靠性较低。两种设置都需要为冗长的技能提示词和不断增长的历史记录支付重复的开销。我们提出了恒定上下文技能学习（constant-context skill learning），这是一种针对循环智能体工作流的“上下文到权重”（context-to-weights）框架：可重用的程序在轻量级任务族模块中学习，而推理仅以当前观测和紧凑的状态块为条件。一个确定性追踪器根据任务进度渲染该状态块，并提供对齐的子目标奖励，因此每个模块都可以通过步级有监督微调（SFT）进行训练，并通过在线强化学习（RL）进行优化。在 ALFWorld、WebShop 和 SciWorld 上，我们的智能体在 Qwen3-4B、Qwen3-8B 和 Llama-3.1-8B 上均取得了强劲的性能。使用 Qwen3-8B，SFT+RL 在 ALFWorld 上的未见任务成功率达到 89.6%，在 WebShop 上达到 76.8%，在 SciWorld 上的未见任务成功率达到 66.4%。它们达到或超过了已发表的强力智能体训练结果，同时与受控的 ReAct 提示词基线相比，每轮的提示词 Token 减少了 2 到 7 倍，表明程序性上下文可以从提示词转移到权重中。
