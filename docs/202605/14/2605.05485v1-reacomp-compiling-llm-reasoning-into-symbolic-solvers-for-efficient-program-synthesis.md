...
title_zh: ReaComp：将大语言模型推理编译为符号求解器以实现高效程序合成
...
tldr: "本研究提出ReaComp，旨在解决大语言模型在复杂程序合成任务中效率低且不可靠的问题。通过将LLM的推理轨迹编译为可重用的符号程序合成器，该方法在测试阶段无需调用LLM，在PBEBench等基准测试上显著优于传统LLM搜索，不仅大幅降低了推理成本（减少78% Token），还展现了极强的跨领域迁移能力。"
selection_source: fresh_fetch
motivation: 针对大语言模型在处理需要大规模组合搜索的复杂程序合成任务时效率低下且可靠性不足的挑战。
method: 利用编码智能体将少量的推理轨迹编译成基于受限领域特定语言（DSL）的可重用符号程序合成器。
result: "在PBEBench-Hard上达到84.7%的准确率，超越了带测试时缩放的LLM，同时在混合设置下减少了78%的Token消耗。"
conclusion: 推理轨迹可以被编译成高效、可重用的符号求解器，为实现领域通用的求解器归纳提供了一条可扩展且低成本的路径。
---

## 摘要
大语言模型（LLMs）能够解决程序合成任务，但在需要大规模组合搜索的困难实例上仍然效率低下且不可靠。给定一小组推理轨迹，我们利用编程智能体（coding agents）将其编译为受限领域特定语言（DSLs）上的可重用符号程序合成器。生成的求解器在测试时无需调用 LLM，且是强大的独立系统：符号求解器集成在 PBEBench-Lite 上达到了 91.3% 的准确率，在 PBEBench-Hard 上达到了 84.7%，在零 LLM 推理成本下，后者比具有测试时扩展（test-time scaling）的 LLM 高出 16.3 个百分点。它们还补充了 LLM 搜索，在将 PBEBench-Hard 的准确率从 68.4% 提升至 85.8% 的同时，减少了 78% 的 Token 使用量，并在神经符号混合设置下将 SLR-Bench 困难层的准确率从 34.4% 提升至 58.0%。与直接将编程智能体用作逐实例求解器相比，归纳出的求解器具有显著更高的帕累托效率（Pareto-efficient），将微小的一次性构建成本分摊到多次零 Token 执行中。最后，大多数求解器可以零样本迁移到真实的历时语言学任务——预测自然语言数据中的语音变化——在集成模式下达到 80.1% 的准确率，并恢复了一些合理的语言规则。总之，这些结果表明，推理轨迹可以被编译为可重用的符号求解器，这些求解器能直接解决许多任务，在困难案例中补充 LLM 推理，并为通用领域求解器归纳提供了一条可扩展的路径。我们发布了代码和数据以供复现。
