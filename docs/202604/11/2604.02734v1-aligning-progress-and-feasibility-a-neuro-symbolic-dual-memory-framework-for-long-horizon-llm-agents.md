...
## 摘要
大语言模型（LLMs）在具身操纵和网页交互等长程决策任务中展现出强大的潜力。然而，在复杂环境中，智能体经常陷入无休止的试错循环，或偏离主要目标。我们将这些失败归因于两个根本性错误：全局进度偏移（Progress Drift）和局部可行性违背（Feasibility Violation）。现有方法通常尝试使用单一范式同时解决这两个问题。然而，这两项挑战在本质上是不同的：前者依赖于模糊语义规划，而后者则要求严格的逻辑约束和状态验证。这种单一范式方法的固有局限性，为现有模型处理长程任务带来了根本性挑战。基于这一洞察，我们提出了一种神经符号双重记忆框架，将语义进度引导与逻辑可行性验证显式解耦。具体而言，在推理阶段，该框架同步调用两种记忆机制：一方面，基于神经网络的进度记忆从成功轨迹中提取语义蓝图，以引导全局任务推进；另一方面，基于符号逻辑的可行性记忆利用从失败转移中合成的可执行 Python 验证函数，进行严格的逻辑校验。实验表明，该方法在 ALFWorld、WebShop 和 TextCraft 上的表现显著优于现有的竞争基准，同时大幅降低了无效动作率和平均轨迹长度。
