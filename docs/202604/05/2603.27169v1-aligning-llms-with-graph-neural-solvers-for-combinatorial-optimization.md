...
tags: ["query:rl-nlplr"]
score: 7.0
evidence: LLM通过语义理解解决组合优化问题
tldr: 针对大语言模型在处理组合优化问题时难以捕捉复杂关系结构且扩展性差的问题，本文提出AlignOPT框架。该方法通过将LLM的语义理解能力与图神经求解器的结构建模能力相结合，实现了语言语义与结构表示的深度对齐。实验证明，AlignOPT在多种组合优化任务上达到了SOTA水平，并展现出极强的泛化能力，能有效处理未见过的中大型实例。
selection_source: fresh_fetch
motivation: 纯语言模型难以准确捕捉组合优化问题中的复杂关系结构，导致其在处理中大型实例时效果受限。
method: 提出AlignOPT框架，利用LLM编码文本描述并结合图神经求解器显式建模图结构，实现语义与结构的对齐。
result: 在多种组合优化问题上取得了当前最先进的性能，并表现出优异的跨实例泛化能力。
conclusion: 将LLM的语义能力与图神经网络的结构建模相结合，是提升神经组合优化求解器性能和通用性的有效途径。
---

## 摘要
最近的研究表明，通过使用自然语言表示任务和实例，大语言模型（LLMs）在解决组合优化问题（COPs）方面表现出了有效性。然而，纯基于语言的方法难以准确捕捉许多 COP 中固有的复杂关系结构，导致其在处理中型或大型实例时效果较差。为了解决这些局限性，我们提出了 AlignOPT，这是一种将 LLMs 与图神经求解器对齐的新颖方法，旨在学习更具泛化能力的神经 COP 启发式算法。具体而言，AlignOPT 利用 LLMs 的语义理解能力来编码 COP 及其实例的文本描述，同时利用图神经求解器显式建模 COP 实例的底层图结构。我们的方法促进了语言语义与结构表示之间的稳健集成与对齐，从而实现了更准确且可扩展的 COP 解决方案。实验结果表明，AlignOPT 在多种 COP 上均取得了当前最先进的结果，凸显了其在对齐语义和结构表示方面的有效性。特别是，AlignOPT 展现出了强大的泛化能力，能够有效地扩展到以前未见过的 COP 实例。
