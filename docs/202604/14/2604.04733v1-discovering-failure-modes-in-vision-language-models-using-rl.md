...
tags: ["query:rl-nlplr"]
score: 7.0
evidence: 基于强化学习的框架用于发现空间推理中的失败模式
...
## 摘要
视觉语言模型（VLMs）尽管在多模态基准测试中取得了强劲表现，但往往会误解人类能够毫不费力识别的简单视觉概念，例如计数、空间推理和视角理解。先前的研究通过人工方式识别了这些弱点，并发现它们通常源于特定技能的缺陷。然而，这种人工尝试成本高昂、难以扩展，且容易受到人类偏见的影响，往往会为了关注显著对象而忽略细微细节，导致对模型脆弱性的理解不完整。为了解决这些局限性，我们提出了一个基于强化学习（RL）的框架，旨在无需人工干预的情况下，自动发现任何候选 VLM 在给定数据分布上的失效模式或盲点。我们的框架训练了一个提问者智能体，该智能体根据候选 VLM 的响应自适应地生成查询，以诱导其给出错误答案。随着训练的进行，我们的方法通过关注细粒度的视觉细节和不同的技能组合来增加问题的复杂性，从而识别出 36 种 VLM 难以应对的新型失效模式。我们通过展示该框架在各种模型组合中的泛化能力，证明了其广泛的适用性。
