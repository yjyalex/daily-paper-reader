...
tags: ["query:rl-nlplr"]
score: 8.0
evidence: 自我演化中的反馈信号与推理路径正确性
tldr: 针对多模态大模型（MLLM）无监督自我进化中反馈信号质量不稳定、易受模型偏见影响的问题，本文提出CSRS框架。该框架通过回溯重推理机制（RRM）探索长尾推理路径，利用平滑频率奖励（SFR）提供连续反馈信号，并结合视觉语义扰动（VSP）强化逻辑推理。实验证明，CSRS在MathVision等基准测试上显著提升了Qwen2.5-VL-7B的性能，在几何任务中达到SOTA水平。
selection_source: carryover_cache
motivation: 现有的无监督自我进化方法依赖多数投票选择伪标签，容易受到模型内在偏见的影响而导致推理路径错误。
method: 提出CSRS框架，包含回溯重推理机制、平滑频率奖励以及视觉语义扰动，旨在通过连续信号和深度探索优化模型学习。
result: 在MathVision等基准测试中显著提升了Qwen2.5-VL-7B的推理能力，并在几何任务的无监督自我进化中取得SOTA结果。
conclusion: CSRS通过优化反馈机制和推理路径探索，有效稳定了MLLM的无监督自我进化过程，增强了模型的数学逻辑推理能力。
---

## 摘要
在多模态大语言模型（MLLMs）的无监督自我演化中，后训练阶段反馈信号的质量对于稳定且有效的学习至关重要。然而，现有的自我演化方法主要依赖多数投票来选择出现频率最高的输出作为伪黄金答案，这可能源于模型的内在偏见，而无法保证推理路径的客观正确性。为了应对这种退化，我们提出了 MLLM 自我演化中的连续平滑回溯重采样（CSRS）。具体而言，我们引入了一种回溯重推理机制（RRM），使模型能够从锚点开始重新推理，以扩大对长尾推理路径的探索。同时，我们提出了平滑频率奖励（SFR），用连续信号取代二元奖励，并根据采样推理集中答案的频率来校准奖励。此外，结合视觉语义扰动（VSP），CSRS 确保模型优先考虑数学逻辑而非视觉表象。实验结果表明，CSRS 显著提升了 Qwen2.5-VL-7B 在 MathVision 等基准测试上的推理性能。我们在几何任务的无监督自我演化中达到了当前最先进（SOTA）的水平。我们的代码可在 https://github.com/yyy195/CSRS 获取。
