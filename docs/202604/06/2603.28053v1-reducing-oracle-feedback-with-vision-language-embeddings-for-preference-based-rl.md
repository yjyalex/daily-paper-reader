...
## 摘要
基于偏好的强化学习可以通过比较学习有效的奖励函数，但其可扩展性受限于专家反馈的高昂成本。轻量级视觉-语言嵌入（VLE）模型提供了一种更廉价的替代方案，但其噪声输出限制了它们作为独立奖励生成器的有效性。为了应对这一挑战，我们提出了 ROVED，这是一个结合了基于 VLE 的监督与针对性专家反馈的混合框架。我们的方法利用 VLE 生成片段级偏好，并仅针对通过过滤机制识别出的高不确定性样本求助于专家。此外，我们引入了一种参数高效的微调方法，利用获取的专家反馈对 VLE 进行自适应调整，从而以协同方式随时间改进模型。这确保了在保留嵌入模型的可扩展性和专家反馈的准确性的同时，避免了各自的低效性。在多项机器人操纵任务中，ROVED 在减少高达 80% 的专家查询的同时，达到或超过了先前的基于偏好的方法。值得注意的是，调整后的 VLE 具有跨任务的泛化能力，累计节省了高达 90% 的标注成本，突显了将可扩展嵌入与精确专家监督相结合用于基于偏好的强化学习的实用性。
