...
tags: ["query:llm-fo"]
score: 7.0
evidence: 工具集成推理中的效率模式与延迟分析
tldr: 本研究针对大模型工具集成推理（TIR）中的效率瓶颈，指出传统指标无法准确衡量因KV缓存失效和长工具响应导致的推理延迟。为此，作者提出了硬件感知的效率指标PTE（预填充Token等效值），能统一衡量推理与工具调用成本。通过在五个基准测试上的实验，识别了四种低效模式，并发现高PTE成本往往伴随着较低的推理准确率，揭示了工具使用并非越多越好。
