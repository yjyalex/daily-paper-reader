...
tags: ["query:mas-gen"]
score: 7.0
evidence: 自主智能体的结构化程序计划
tldr: 针对LLM智能体缺乏持久程序化记忆、重复解决相似任务的问题，本文提出APEX-EM框架。这是一种非参数化在线学习方法，通过结构化经验表示（包含执行轨迹、错误分析和评分）和PRGII工作流，实现经验的积累与重用。该框架利用双重结果经验记忆，结合语义和结构匹配，显著提升了智能体在代码生成、知识图谱查询等复杂任务上的性能，无需修改模型权重。
selection_source: fresh_fetch
motivation: 现有的LLM智能体在面对结构相似的任务时，往往无法复用既往经验，导致重复推导和效率低下。
method: 提出一种包含结构化经验表示、PRGII工作流及双重结果经验记忆的非参数化在线学习框架，通过检索正负样本进行上下文学习。
result: "在KGQAGen-10k和BigCodeBench等基准测试中，该方法分别实现了48.3%和29.4%的显著性能提升，超越了现有基准和Oracle检索上限。"
conclusion: APEX-EM证明了结构化程序化记忆在提升智能体跨领域迁移和复杂任务处理能力方面的巨大潜力，且各组件效能具有任务依赖性。
---
