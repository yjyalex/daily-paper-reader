...
tags: ["query:llm-fo"]
score: 7.0
evidence: 推理搜索与计算预算之间的权衡
tldr: 针对LLM智能体在推理时过度消耗资源的问题，本文提出BAVT框架。这是一种无需训练的推理时搜索方法，通过动态搜索树和步级价值评估引导推理。其核心是预算感知的节点选择和残差价值预测，实现了从探索到利用的平滑过渡并有效剪枝。实验证明，BAVT在低预算下表现优于4倍资源的基线，显著提升了推理效率。
