...
tags: ["query:rl-nlplr"]
score: 6.0
evidence: 用于具身任务规划的分阶段学习与强化学习
tldr: 针对具身任务规划中视觉定位与因果一致性的挑战，本文提出分阶段视觉语言学习框架（SVLL）。该框架通过三阶段训练解耦空间定位与时间推理，并引入Bias-DPO对齐算法，通过注入专家轨迹偏置来解决传统DPO在物理约束下的幻觉问题。实验证明SVLL在AI2-THOR及真实机器人任务中表现优于GPT-4o等模型，显著提升了成功率并减少了违规行为。
selection_source: fresh_fetch
motivation: 现有的具身规划训练方法在处理空间定位与时间推理的耦合时存在过早绑定或优化不稳定的问题，且标准DPO缺乏对绝对最优路径的约束。
method: 提出SVLL框架，前两阶段解耦空间与时间学习，第三阶段采用Bias-DPO算法，在优化偏好间隙的同时最大化专家轨迹的似然概率。
result: 在AI2-THOR基准测试和真实机器人部署中，SVLL的成功率超越了Qwen2.5-VL、GPT-4o和Gemini-2.0-flash等主流模型。
conclusion: 通过分阶段学习和改进的对齐目标，SVLL有效增强了具身智能体的物理接地能力，并显著抑制了物理上不可行的幻觉行为。
---
