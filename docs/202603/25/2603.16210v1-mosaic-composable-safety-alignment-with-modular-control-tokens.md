...
title_zh: MOSAIC：基于模块化控制标记的可组合安全对齐
...
## 摘要
大语言模型（LLM）中的安全对齐通常被实现为嵌入在模型参数中的单一静态策略。然而，实际部署往往需要依赖上下文的安全规则，这些规则因用户、地区和应用而异。现有方法难以提供此类条件控制：参数级对齐将安全行为与通用能力纠缠在一起，而基于提示的方法依赖于自然语言指令，其强制执行力较弱。我们提出了 MOSAIC，这是一个模块化框架，通过在冻结的主干模型上优化可学习的控制标记，实现可组合的安全对齐。每个标记代表一个安全约束，并可以在推理时灵活地激活和组合。为了高效地训练组合标记，我们引入了基于顺序的任务采样和一种减轻过度拒绝的分布级对齐目标。实验表明，MOSAIC 在保持模型效用的同时，实现了强大的防御性能，并显著降低了过度拒绝率。
