...
## 摘要
基于大语言模型（LLM）的智能体在短程且高度结构化的任务中取得了显著成功。然而，在真实且动态的环境中，它们在长程任务中保持连贯决策的能力仍然是一个开放性挑战。我们推出了 RetailBench，这是一个高保真基准测试，旨在评估真实商业场景下的长程自主决策，其中智能体必须在随机需求和不断变化的外部条件下运行。我们进一步提出了“演化策略与执行”（Evolving Strategy & Execution）框架，该框架将高层战略推理与底层动作执行相分离。这种设计实现了随时间变化的自适应且可解释的策略演化。这对于长程任务尤为重要，因为在非平稳环境和误差累积的情况下，策略修订的时间尺度需要与动作执行的时间尺度不同。在八种最先进的 LLM 上进行的实验表明，在难度递增的环境中，与其它基准方法相比，我们的框架提高了运营稳定性和效率。然而，随着任务复杂度的增加，性能显著下降，揭示了当前 LLM 在长程、多因素决策方面的根本局限性。
