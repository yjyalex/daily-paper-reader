...
## 速览
**TLDR**：针对大语言模型在复杂推理中存在的“过度思考”和推理效率低下的问题，本文提出了弱监督组相对策略优化（WS-GRPO）。该方法通过将终端奖励转化为前缀级的正确性感知引导，训练一个偏好模型来判断何时继续或停止推理。实验证明，WS-GRPO 在保持推理准确性的同时，显著缩短了生成长度，提高了推理效率。 \
**Motivation**：现有的 GRPO 方法容易导致模型为了获得相对增益而产生冗余推理，且传统的长度惩罚难以在推理质量与效率之间取得平衡。 \
**Method**：提出 WS-GRPO，利用仅有的结果正确性训练偏好模型，为部分轨迹提供前缀级的“继续/停止”引导信号。 \
**Result**：在推理基准测试中，WS-GRPO 在保持与基准模型相当的准确率的同时，大幅减少了推理过程中的生成长度。 \
**Conclusion**：WS-GRPO 是一种有效的弱监督方法，能够通过结果导向的引导显著提升大语言模型推理的效率和简洁性。
