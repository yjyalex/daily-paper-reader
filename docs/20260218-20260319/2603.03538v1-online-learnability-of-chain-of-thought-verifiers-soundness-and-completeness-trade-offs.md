...
## 速览
**TLDR**：针对大语言模型在思维链推理中易出错的问题，本文提出了一个在线学习框架来训练思维链验证器。研究重点探讨了验证器在可靠性（漏报错误）与完备性（误报正确）之间的权衡，并引入了Littlestone维度的扩展来刻画学习过程中的错误界限。通过优化算法寻找帕累托前沿，该框架能显著提升弱证明器的准确性，并实现超越训练分布的证明生成能力。 \
**Motivation**：解决大语言模型在复杂推理中容易出错以及验证器在反馈循环中面临的分布偏移挑战。 \
**Method**：提出了一个在线学习框架，通过扩展Littlestone维度来量化可靠性与完备性错误，并设计了寻找帕累托前沿的最优算法。 \
**Result**：理论上给出了验证器学习的错误界限，并证明了该验证器能有效提升弱证明器的性能，使其具备生成更强证明的能力。 \
**Conclusion**：该研究为思维链验证器的在线学习提供了严谨的理论基础，展示了通过权衡不同类型错误来增强模型推理可靠性的可行性。
