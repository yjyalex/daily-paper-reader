...
## 速览
**TLDR**：TableMind++ 针对表格推理中 LLM 随机性导致的幻觉问题，在 TableMind 基础上引入了不确定性感知推理框架。它通过记忆引导的计划剪枝、基于置信度的动作细化以及双权重轨迹聚合，有效缓解了认知和偶然不确定性。该方法结合了自主训练与不确定性量化，在多个基准测试中超越了现有基准和商业模型，显著提升了轻量级模型在复杂表格任务中的数值精度和逻辑稳定性。 \
**Motivation**：针对现有表格推理模型在处理复杂数值运算时因 LLM 随机性而产生的幻觉和逻辑错误问题。 \
**Method**：提出一种不确定性感知框架，包含记忆引导的计划剪枝、基于置信度的动作细化及双权重轨迹聚合。 \
**Result**：在多个多样化基准测试中，TableMind++ 的表现一致优于先前的基准模型和闭源商业模型。 \
**Conclusion**：结合自主训练与不确定性量化能显著增强程序化智能体在工具增强型表格推理任务中的鲁棒性和准确性。
