...
## 速览
**TLDR**：针对视觉语言过程奖励模型（VL-PRM）在感知与推理纠缠导致的评分不可靠问题，本文提出显式视觉前提验证（EVPV）框架。该方法通过生成步骤级的视觉清单并与独立提取的结构化视觉约束进行匹配，计算视觉可靠性信号来校准奖励分数。EVPV有效解耦了感知不确定性与逻辑评估，在多个多模态推理基准测试中显著提升了验证准确率和Best-of-N重排序性能。 \
**Motivation**：现有的VL-PRM作为黑盒评分器，难以区分推理错误与视觉感知错误，导致评分存在严重的幻觉奖励或错误惩罚。 \
**Method**：通过生成步骤级视觉清单并与从图像中独立提取的结构化约束进行匹配，计算可靠性信号来动态校准PRM的步骤奖励。 \
**Result**：在VisualProcessBench及六个多模态推理基准上，EVPV显著提升了步骤级验证精度和Best-of-N重排序的准确率。 \
**Conclusion**：显式验证视觉前提能有效解耦感知与推理，为构建更可靠、可解释的多模态过程奖励模型提供了有效路径。
