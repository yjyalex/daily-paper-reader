...
## 速览
**TLDR**：针对检索增强智能体在多步推理中因噪声检索和奖励信号粗糙导致的可靠性问题，本文提出 EvalAct 框架。该框架将检索质量评估转化为显式动作，通过“搜索-评估”协议为每步检索提供即时过程信号。结合基于 GRPO 的 PCAR 优化方法，根据评估分数重缩放优势值，显著提升了模型在多跳问答等任务上的性能和可靠性。 \
**Motivation**：现有的检索增强智能体在多步推理中易受噪声干扰，且传统的仅基于结果的强化学习难以对中间推理步骤提供精细的优化信号。 \
**Method**：提出 EvalAct 框架，将检索评估设为显式动作，并引入 PCAR 优化方法，根据过程评估分数动态调整强化学习中的优势值。 \
**Result**：在七个开放域问答基准测试中，EvalAct 取得了最佳平均准确率，特别是在需要多步推理的复杂任务中表现出显著优势。 \
**Conclusion**：通过将隐式评估显式化并结合过程校准的优化策略，可以有效增强检索增强智能体在处理复杂多跳问题时的鲁棒性。
