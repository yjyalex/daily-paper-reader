...
tags: ["query:rl-nlplr"]
score: 9.0
evidence: 由LLM子目标分解引导的强化学习智能体
tldr: 针对强化学习在稀疏奖励环境下采样效率低的问题，本文提出MIRA框架。该框架通过构建一个整合了LLM先验知识和高回报经验的结构化记忆图，在训练初期引导智能体。通过将LLM建议转化为软性优势估计调整，MIRA在显著减少LLM调用次数的同时，提升了学习速度，并最终通过效用衰减保证了算法的收敛性。
selection_source: fresh_fetch
motivation: 旨在解决强化学习在稀疏奖励任务中采样复杂度高，以及过度依赖大语言模型（LLM）实时指导带来的成本和可靠性问题。
method: 提出一种记忆集成强化学习智能体，利用LLM和高回报轨迹构建记忆图，并通过软性调整优势估计的效用信号来引导策略更新。
result: 实验表明，MIRA在稀疏奖励环境下优于传统RL基准，且在大幅减少LLM在线查询次数的情况下，达到了与高频LLM指导方法相当的性能。
conclusion: MIRA证明了通过结构化记忆有效摊销LLM知识，可以在保证收敛性的前提下，显著提升强化学习的早期学习效率和可扩展性。
---
