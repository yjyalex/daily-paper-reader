...
## 速览
**TLDR**：针对大语言模型在复杂推理中因缺乏自我调节而导致的结构性脆弱问题，本文提出了元认知行为微调（MBT）框架。该框架通过合成严谨推理轨迹（MBT-S）和重写初始轨迹（MBT-R）两种方式，将人类般的元认知策略注入模型。实验证明，MBT能有效消除推理崩溃，在提升多跳问答准确率的同时显著降低Token消耗，增强了推理的稳定性和鲁棒性。 \
**Motivation**：研究发现大模型推理失败往往源于缺乏自我调节控制而非推理能力不足，导致逻辑链条在探索过程中容易失稳。 \
**Method**：提出元认知行为微调（MBT）框架，通过合成严谨轨迹（MBT-S）和重写初始轨迹（MBT-R）将元认知行为注入模型推理过程。 \
**Result**：在多跳问答基准测试中，MBT显著优于基准模型，在提高准确率的同时大幅减少了推理所需的Token数量。 \
**Conclusion**：将元认知策略内化到模型中可以有效消除推理崩溃，使大语言模型的推理过程更加稳定、高效且鲁棒。
