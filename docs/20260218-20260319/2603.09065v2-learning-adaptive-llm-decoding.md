...
**Method**：引入轻量级适配器，通过强化学习在序列级（上下文多臂老虎机）和标记级（POMDP）动态调整采样策略。 \
**Result**：在MATH和CodeContests基准上，该方法显著优化了准确率与预算的权衡，其中标记级适配器在固定预算下将Pass@1准确率提升了高达10.2%。 \
**Conclusion**：实验证明，通过序列级和标记级的自适应解码，可以在不微调底座模型的情况下有效提升LLM的推理性能和资源利用率。
