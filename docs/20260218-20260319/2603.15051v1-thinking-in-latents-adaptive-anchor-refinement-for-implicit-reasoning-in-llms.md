...
tags: ["query:rl-nlplr"]
score: 8.0
evidence: 作为Token级思维链替代方案的隐空间推理
tldr: 针对大语言模型思维链推理成本高的问题，本文提出 AdaAnchor 框架。该方法通过在潜空间迭代优化锚点向量进行隐式推理，并引入自适应停止机制，根据锚点稳定性动态调整计算步数。实验显示，AdaAnchor 在显著减少 Token 生成量的同时，比固定步数方法更准确且更高效，实现了推理精度与计算成本的优异平衡。
selection_source: fresh_fetch
motivation: 现有的潜空间推理方法多采用固定迭代步数，无法针对任务难度灵活分配计算资源。
method: 提出 AdaAnchor 框架，通过优化潜锚点向量进行隐式推理，并利用自适应停止机制动态终止迭代。
result: "在数学推理任务中，该方法比固定步数方案准确率提升达 5% 且步数减少约 50%，同时大幅削减了 Token 生成量。"
conclusion: AdaAnchor 证明了自适应潜空间推理是提升大语言模型推理效率并保持高准确性的有效途径。
---

## 摘要
令牌级思维链（CoT）提示已成为在大语言模型（LLM）中引导多步推理的标准方法，尤其是在解决数学应用题方面。然而，生成冗长的中间轨迹会增加输出长度和推理成本，且当模型无需大量文字表述即可得出正确答案时，这种方式可能效率低下。这促使了潜空间推理方法的发展，该方法将计算转移到隐藏表示中，并仅输出最终答案。然而，许多潜空间推理方法在推理时依赖于固定数量的潜空间优化步骤，这增加了一个必须在不同模型和数据集上进行调优的超参数，以平衡准确性和效率。我们提出了 AdaAnchor，这是一个潜空间推理框架，通过优化附加在输入上的一组潜锚点向量来执行静默迭代计算。AdaAnchor 进一步结合了自适应停止机制，该机制监控迭代过程中的锚点稳定性，并在锚点动态收敛时终止优化，从而在共享的最大步数预算下，为简单实例分配较少的步骤，而为困难实例保留更多的优化步骤。我们在三个数学应用题基准测试上的实证评估表明，带有自适应停止机制的 AdaAnchor 相比固定步数的潜空间优化，准确率提升了高达 5%，同时在相同的最大步数预算下，平均潜空间优化步数减少了 48-60%。与标准推理基准相比，AdaAnchor 通过将计算转移到静默潜空间优化中，大幅减少了生成的令牌数量（92-93%），在显著降低输出令牌使用量的同时，提供了一种不同的准确性与效率权衡方案。
