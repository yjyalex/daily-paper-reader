...
tags: ["query:rl-nlplr"]
score: 9.0
evidence: 低共识输入通过强化学习进行优化
tldr: 本研究针对大语言模型推理任务中测试时微调（TTA）效率低且不稳定的问题，提出了DiSCTT框架。该框架利用采样轨迹的一致性来评估实例难度，对高一致性样本采用伪标签监督微调，对低一致性样本采用共识正则化的强化学习。实验证明，DiSCTT在多个推理基准上显著提升了准确率，并大幅减少了计算开销和训练时间，实现了更高效的推理能力增强。
selection_source: fresh_fetch
motivation: 现有的测试时微调方法对所有输入采用统一优化目标，导致在处理异构推理问题时效率低下且不够稳定。
method: 提出一种难度感知的自课程学习框架，根据采样轨迹的共识程度动态分配监督微调或强化学习优化策略。
result: 在数学和通用推理基准测试中，该方法在提高准确率的同时，显著降低了计算资源消耗和训练耗时。
conclusion: 显式考虑实例难度和不确定性是实现稳定、高效且有效的推理模型测试时微调的关键。
---
