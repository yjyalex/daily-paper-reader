...
tags: ["query:rl-nlplr"]
score: 10.0
evidence: 基于过程的奖励信号和基于GRPO的多步推理优化
...
motivation: 现有的检索增强智能体在多步推理中易受噪声干扰，且传统的仅基于结果的强化学习难以提供细粒度的中间步骤优化信号。
method: 提出 EvalAct 框架，将检索评估设为显式动作，并引入 PCAR 优化方法，根据过程评估分数动态调整强化学习中的优势值。
result: 在七个开放域问答基准测试中，EvalAct 取得了最佳平均准确率，特别是在需要多步推理的复杂任务中表现优异。
conclusion: 通过将隐式评估显式化并结合过程校准的优化策略，可以显著增强检索增强智能体在处理复杂多跳问题时的鲁棒性。
---
