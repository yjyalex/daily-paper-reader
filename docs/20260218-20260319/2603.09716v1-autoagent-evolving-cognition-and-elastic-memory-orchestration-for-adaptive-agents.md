...
tldr: 针对自主智能体在长期学习与实时决策中的失衡问题，本文提出 AutoAgent 框架。该框架通过演化认知、即时决策和弹性存储编排三大核心组件，实现了智能体认知的自我进化与高效的长程推理。AutoAgent 无需外部重训，通过闭环认知进化过程对工具、能力及任务知识进行动态更新，显著提升了智能体在开放动态环境中的适应性与协作鲁棒性。
selection_source: fresh_fetch
motivation: 现有智能体框架因认知静态、工作流僵化及上下文利用效率低，难以在非平稳环境中实现高效的长期学习与实时决策。
method: 提出一种集成结构化认知演化、统一动作空间选择及动态压缩与抽象历史记录的弹性存储编排器的多智能体框架。
result: 在检索增强推理、工具调用及具身任务等多个基准测试中，AutoAgent 在任务成功率、工具使用效率和协作稳定性上均优于基准模型。
