...
## 摘要
我们提出了一种资源高效的框架，通过知识蒸馏结合引导式思维链（chain-of-thought）强化学习来压缩大语言模型。以 Qwen 3B 为教师模型，Qwen 0.5B 为学生模型，我们在英文 Dolly-15k、西班牙文 Dolly-15k 以及代码数据集 BugNet 和 PyTorrent 上应用了知识蒸馏，并在英文环境下调整超参数以优化学生模型的性能。在各项任务中，蒸馏后的学生模型在保持显著更小体积的同时，保留了教师模型的大部分能力：在英文任务中保留了 70% 至 91%，在西班牙文任务中高达 95%，在代码任务中的 Rouge-L 指标高达 93.5%。对于编程任务，将思维链提示与基于 CoT 标注的 Codeforces 数据进行的群体相对策略优化（GRPO）相结合，相比仅使用知识蒸馏，提高了推理的连贯性和解题的正确性。训练后的 4 位权重量化进一步减少了内存占用和推理延迟。这些结果表明，知识蒸馏结合思维链引导的强化学习可以产生紧凑、高效的模型，适用于资源受限环境下的部署。
