...
motivation: 旨在解决大语言模型在表格推理过程中因随机性导致的幻觉问题，以及传统单轮推理模式下的上下文溢出和数值计算弱点。
...
## 摘要
表格推理要求模型同时具备语义理解和精确的数值运算能力。大多数现有方法依赖于针对表格的单轮推理范式，这面临着上下文溢出和数值敏感性弱的问题。为了解决这些局限性，我们此前提出了 TableMind，这是一种基于微调的自主程序化智能体，能够在轻量级大语言模型（LLM）中模拟类人交互。TableMind 通过两阶段训练策略将规划、行动和反思内化，该策略包括在筛选的高质量数据上进行有监督微调（SFT），以及通过多视角奖励和秩感知策略优化（RAPO）算法进行的强化学习（RL）。尽管 TableMind 为程序化智能体奠定了坚实基础，但 LLM 固有的随机性仍然是导致幻觉的关键挑战。在本文中，我们通过引入一种新型的不确定性感知推理框架来缓解幻觉，从而将这一基础扩展为 TableMind++。具体而言，我们提出了记忆引导的规划剪枝，通过检索历史轨迹来验证并过滤掉逻辑有误的规划，以应对认知不确定性（epistemic uncertainty）。为了确保执行精度，我们引入了基于置信度的动作细化，通过监控 Token 级概率来检测并自我修正语法噪声，从而缓解偶然不确定性（aleatoric uncertainty）。最后，我们采用双加权轨迹聚合，从多条推理路径中合成稳健的共识。在多个基准测试上的广泛实验表明，TableMind++ 持续优于之前的基准模型和专有模型，验证了将自主训练与不确定性量化相结合的有效性。我们的代码已开源。
