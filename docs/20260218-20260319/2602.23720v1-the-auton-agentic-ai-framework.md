...
tags: ["query:llm-fo"]
score: 8.0
evidence: 随机LLM输出与确定性模式一致输入之间的不匹配
tldr: 本文提出了 Auton Agentic AI 框架，旨在解决生成式 AI 向代理式 AI 转型中，随机性 LLM 输出与确定性后端系统之间的架构失配问题。该框架通过分离认知蓝图与运行引擎，实现了跨语言移植和形式化审计。其核心贡献包括基于增强 POMDP 的执行模型、分层记忆架构、约束流形安全机制及三级自进化框架，显著提升了自主代理系统的标准化、安全性和执行效率。
selection_source: fresh_fetch
motivation: 解决大语言模型随机非结构化输出与后端基础设施所需的确定性、模式一致性输入之间的架构不匹配问题。
method: 提出一种分离认知蓝图与运行引擎的架构，结合增强 POMDP 模型、分层记忆系统和约束流形安全策略。
result: 实现了具备跨语言移植性、形式化审计能力和低延迟执行效率的标准化自主代理系统。
conclusion: Auton 框架为自主代理系统的构建、执行和治理提供了一套原则性的、可扩展且安全的架构方案。
---
